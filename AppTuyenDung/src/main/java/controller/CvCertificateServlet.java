...
import service.CvCertificateService;
...
@WebServlet(urlPatterns = {"/cv-certificates/*"})
public class CvCertificateServlet extends BaseServlet {

    private final CvCertificateService cvCertificateService = new CvCertificateService();
    ...
    // Chỉ candidate sở hữu CV mới được sửa danh sách chứng chỉ gắn vào CV đó
    private boolean isOwnerOfCv(HttpServletRequest req, int cvId) {
        ...
        Integer cvCandidateId = cvCertificateService.getCandidateIdOfCv(cvId);
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            List<CvCertificate> list = cvCertificateService.getByCvId(cvId);

            ApiResponse<List<CvCertificate>> response =
                    new ApiResponse<>(true, "Lấy danh sách chứng chỉ gắn CV thành công!", list);
    ...
    // PUT /cv-certificates — body: { "cvId": 1, "candidateCertificateIds": [1,2] }
    @Override
    protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            List<Integer> candidateCertificateIds = ((List<Number>) body.getOrDefault("candidateCertificateIds", List.of()))
                    .stream().map(Number::intValue).collect(java.util.stream.Collectors.toList());

            cvCertificateService.replaceForCv(cvId, candidateCertificateIds);

            ApiResponse<Object> response = new ApiResponse<>(true, "Cập nhật chứng chỉ cho CV thành công!", null);
